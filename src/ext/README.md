...
7. state 名必须带动作 id 前缀（如 `fish.cast`），且**不得与内置状态同名**（`ctx.enter('walk')` 会以内置逻辑跑，内部数据全是错的）。内置睡觉态判定是精确匹配 `sleepin/sleeping/sleepout`（`isSleepState()`），带 `sleep` 前缀的扩展状态名（如 `sleepwalk.walk`）不会被误当睡觉系。
